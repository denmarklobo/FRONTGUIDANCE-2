<template>
    ...
    <v-list-item-group v-for="student in uniqueStudentIds" :key="student.student_id">
      <v-list-item @click="viewStudentViolations(student.student_id)">
        <v-list-item-content>
          <v-list-item-title class="student-list-item">
            <span class="student-id">{{ student.student_id }}</span>
            <span class="student-name">{{ student.full_name }}</span>
            <span :class="caseStatusClass(student.student_id)">
            ...
            <v-list-item-group v-for="(caseItem, index) in selectedStudentViolations" :key="index">
              <v-list-item>
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle><strong>Status:</strong> {{ caseItem.case_status === 0 ? 'Not Cleared' : 'Cleared' }}</v-list-item-subtitle>
                  <v-list-item-subtitle><strong>Date:</strong> {{ caseItem.case_date }}</v-list-item-subtitle>
                </v-list-item-content>
                <v-list-item-action>
  ...
  export default {
    ...
    return this.cases.map(violation => ({
      ...violation,
      case_status: violation.case_status === 0 ? 'Not Cleared' : 'Cleared',
    })).filter(violation =>
  ...
  .student-id, .student-name, .case-status {
